...
class HomeCooldown extends Task
{
    ...
    public function onRun(): void
    {
        ...
        $effect = $this->player->getEffects()->get(VanillaEffects::BLINDNESS());
        if ($this->lastPosition->subtractVector($this->player->getPosition())->lengthSquared() > 2) {
            ...
            if($effect && $effect->getAmplifier() === 255)
        ...
        if ($this->timer > 0) {
            ...
            if(!$effect)
                $this->player->getEffects()->add(new EffectInstance(VanillaEffects::BLINDNESS(), 1000000, 255, false));
            return;
        }
        if($effect && $effect->getAmplifier() === 255)
            $this->player->getEffects()->remove(VanillaEffects::BLINDNESS());
    ...
    private static function addPlayerInList(Player $player): void
    {
